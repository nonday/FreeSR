...
#ifndef SPEAKER_RECOGNIZER_GMM_H_
#define SPEAKER_RECOGNIZER_GMM_H_
...
class DLL_API SpeakerRecognizerGMM : public SpeakerRecognizer
{
public:
	SpeakerRecognizerGMM(int sample_rate = 16000);

public:
	virtual ~SpeakerRecognizerGMM();

private:
	SpeakerRecognizerGMM(const SpeakerRecognizerGMM &);
	SpeakerRecognizerGMM & operator=(const SpeakerRecognizerGMM &);
...
private:
	...
	Gmm ubm_;
	// 已注册的模型
	std::vector<Gmm> reg_models_;
	// factor: gmm adapt from ubm
	double map_factor_;
};

#endif // SPEAKER_RECOGNIZER_GMM_H_
