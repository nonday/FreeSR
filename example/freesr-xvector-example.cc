...
int main(int argc, char** argv)
{
	...
	SpeakerRecognizerXVector spk(sr);
	bool ret = spk.Init("freesr-xvector-model.bin");
	if (!ret)
	{
	...
	ret = spk.RegisterSpeaker(signal, "english", "english.xv");
	if (!ret)
	{
	...
	ret = spk.RegisterSpeaker({"english.wav"}, "english", "english.xv");
	if (!ret)
	{
		std::cout << "Failed to Register ..." << std::endl;
		return -1;
